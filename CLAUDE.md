...
- **Sound**: all SFX are synthesized with Web Audio (`playMeow`/`playPop`/`playWarning`) — no audio assets. The AudioContext is created/resumed by a one-time `mousedown`/`touchstart` listener because iOS only allows audio to start inside a user gesture, and the single-click path runs in a `setTimeout` (not a gesture context). Auto-marks after a cat reveal are intentionally silent (only the meow plays).
