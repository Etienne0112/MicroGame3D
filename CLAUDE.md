...
- **Rendering is full re-render**: `render()` rebuilds the entire grid DOM on every state change (max 10×10 = 100 cells, so this is fine). Don't add incremental DOM updates without reason. Region boundaries are drawn as thick borders via `bt/bb/bl/br` classes computed from neighboring cells' region ids.
...
- **Drag-marking**: mousedown fixes the drag mode (the toggle result of the start cell — mark or erase); dragging applies that mode to every cell entered, skipping revealed diamonds and `'wrong'` cells. After a real drag, `suppressClick` swallows the click event that follows mouseup so the start cell isn't toggled twice.
